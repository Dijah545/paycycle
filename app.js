...
function renderDashboardCharts(m){
  ...
  $('#cashFlowStatus').className=`status-chip ${net>=0?'positive':'negative'}`;
  ...
  if(!total){
    $('#spendDonut').style.background='#e2e8f0';
    $('#donutLegend').innerHTML='<p class="muted small">No expenses in this period yet.</p>';
    return;
  }
  let cursor=0,parts=[],legend=[];
  entries.slice(0,6).forEach(([name,value],i)=>{
    const pct=value/total*100,end=cursor+pct,color=palette[i%palette.length];
    parts.push(`${color} ${cursor}% ${end}%`);cursor=end;
    legend.push(`<div class="legend-item"><i class="legend-dot" style="background:${color}"></i><span>${esc(name)}</span><strong>${Math.round(pct)}%</strong></div>`);
  });
...
function renderRecentTransactions(m){
  ...
  $('#recentTransactions').innerHTML=rows.length?rows.map(t=>{
    ...
    if(t.type==='income'){title=t.description;meta=`Income → ${accountName(t.accountId)}`;amt='+'+currency.format(t.amount);cls='positive'}
    if(t.type==='expense'){title=t.description;meta=`${budgetName(t.category)} • ${accountName(t.accountId)}`;amt='−'+currency.format(t.amount);cls='negative'}
    ...
    return`<div class="tx"><div><strong>${esc(title)}</strong><div class="tx-meta">${esc(meta)} • ${esc(t.date)}</div></div><strong class="${cls}">${amt}</strong></div>`
  }).join(''):'<p class="muted">No activity in this period yet.</p>';
}
