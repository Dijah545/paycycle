...
function renderDashboardCharts(m){
  ...
  const palette=['#008b68','#e71d4f','#1769e0','#5b35d5','#ff8a00','#6b7280'];
...
$('#exportBtn').onclick=()=>download(`khadijahs-budget-backup-${today()}.json`,JSON.stringify(state,null,2),'application/json');
...
$('#resetBtn').onclick=async()=>{if(confirm('This will erase all Khadijah's Budget data stored in this browser. Export a backup first if needed. Continue?')){state={settings:{payday:25,holidays:[],autoJamaicaHolidays:true},accounts:[],transactions:[],budgets:[],commitments:[],funds:[],plannedIncome:[],debts:[],billStatus:{}};periodOffset=0;await commit()}};
function csvCell(v){return`"${String(v??'').replaceAll('"','""')}"`}
$('#exportCsvBtn').onclick=()=>{const h=['Date','Type','Description','Category','Amount','Account','From Account','To Account','Notes'],rows=[h,...state.transactions.map(t=>[t.date,t.type,t.description||'',t.category||'',t.amount,t.accountId?accountName(t.accountId):'',t.fromAccountId?accountName(t.fromAccountId):'',t.toAccountId?accountName(t.toAccountId):'',t.notes||''])];download(`khadijahs-budget-transactions-${today()}.csv`,rows.map(r=>r.map(csvCell).join(',')).join('\n'),'text/csv')};
$('#exportSummaryCsvBtn').onclick=()=>{const m=currentMetrics(),rows=[['Metric','Value'],['Period',`${fmtDate(m.cycle.start)} - ${fmtDate(m.cycle.end)}`],['Income',m.income],['Expenses',m.spent],['Net Cash Flow',m.income-m.spent],['Unpaid Commitments',m.unpaidTotal],['Reserved Savings',m.reserved],['Debt Balances',m.debt],['Net Financial Position',m.net],['Free to Spend',m.free]];download(`khadijahs-budget-period-summary-${cycleKey(m.cycle)}.csv`,rows.map(r=>r.map(csvCell).join(',')).join('\n'),'text/csv')};
